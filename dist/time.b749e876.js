...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"js/time.js":[function(require,module,exports) {
...
var timeMenuToggleOpen = false; // toggling whether the dropdown menu isopen or closed
...
var stopwatchSelector = document.getElementById('stopwatch-select');
var pomodoro = document.getElementById('pomodoro');
var pomodoroSelector = document.getElementById('pomodoro-select'); // timer dropdown selector
...
function setTimerType() {
  var currentTimer = timeSelector.querySelector(':first-child');
...
var stopwatchStart = document.querySelector('#stopwatch button.start-stop');
var stopwatchReset = document.querySelector('#stopwatch button.reset');
var stopwatchOn = false,
    intervals; // resetting the stopwatch
...
stopwatchReset.addEventListener('click', function () {
  ...
  peekSeconds.textContent = round(sec); // reset inner text of button

  stopwatchStart.textContent = 'Start';
  stopwatchStart.classList.remove('danger');
  stopwatchStart.classList.add('primary'); // enable reset button clicking

  stopwatchReset.disabled = false;
  stopwatchOn = false;
  timeToolsOverlay.setAttribute('static', 'true'); // turn off counting

  clearInterval(intervals); // enable dropdown

  dropdown.addEventListener('click', toggleTimeMenu);
});
var start, currentTime, elapsedTime, stringify;
var offset = 0; // stopwatch start/stop button

stopwatchStart.addEventListener('click', function () {
  timeToolsOverlay.setAttribute('static', 'false'); // if the stopwatch is on, we want to turn it off
  ...
  if (stopwatchOn == true) {
    ...
    stopwatchOn = false;
    offset = parseInt(milli) * 10; // otherwise, if the stopwatch is not on,
  } else if (stopwatchOn == false) {
    start = Date.now();
    incrementUp(); // increment the time upwards from the time the start button was clicked
    // learnt from https://stackoverflow.com/questions/29971898/how-to-create-an-accurate-timer-in-javascript

    function incrementUp() {
      currentTime = Date.now();
      elapsedTime = currentTime - start + offset; // formatting the elapsed time

      stringify = elapsedTime.toLocaleString('en-US', {
        minimumIntegerDigits: 3,
        useGrouping: false
      });
      milli = stringify[0] + stringify[1]; // incrementing seconds
    ...
    stopwatchReset.disabled = false; // replace text content for relevance and restyle button

    stopwatchStart.textContent = 'Stop';
    stopwatchStart.classList.remove('primary');
    stopwatchStart.classList.add('danger'); // disable the timer dropdown
...
var pomoAlarm = document.getElementById('alarm-sound');
var workButtons = document.querySelector('#pomodoro #work'),
...
var pomoIntervals,
    pomoOn = false; //es6 shorthand for event listeners https://josephcardillo.medium.com/arrow-functions-and-this-in-es6-4f1d350a85cf

workAdd.addEventListener('click', function () {
  return pomoTime('add', 'work');
});
workSubtract.addEventListener('click', function () {
  return pomoTime('subtract', 'work');
});
breakAdd.addEventListener('click', function () {
  return pomoTime('add', 'break');
});
breakSubtract.addEventListener('click', function () {
  return pomoTime('subtract', 'break');
});
workButtons.style.display = 'flex';
breakButtons.style.display = 'flex';
timer.style.display = 'none'; // timeline is the visual representation of work and break times in a pomodoro timer

var timeline = document.getElementById('timeline');
var loadSpans = document.querySelectorAll('#pomodoro div.length > div'),
    ...
    resetPomo = document.getElementById('reset-pomo'); // reset button to return to default 
...
pomoStartStop.addEventListener('click', function () {
  ...
  if (pomoOn == true) {
    ...
    // timer is active
    timeToolsOverlay.setAttribute('static', 'false'); // increment downwards (uses similar functionality to stopwatch above^^)
    ...
    timer.style.display = 'flex'; // set intervals and text content
...
var workSpans = document.querySelectorAll('#pomodoro div > div.length:nth-child(2n + 1)'),
    breakSpans = document.querySelectorAll('#pomodoro div > div.length:nth-child(2n)'),
    pomoStatus = document.getElementById('pomo-status');
var phase = 'work',
    sessions = 0; // increments timer times up and down
...
function pomoTime(operator, mode) {
  ...
  if (mode == 'work') {
    ...
    if (operator == 'add' && workMin < 60) {
      workMin += incrementValue; // decrease
    ...
    if (operator == 'add' && breakMin < 20) {
      breakMin += incrementValue; // decrease
...
function pomoLength() {
  ...
  var total = workMin * 3 + breakMin * 2,
      timelineLength = 232,
      workLength = workMin / total * timelineLength,
      breakLength = breakMin / total * timelineLength; // apply relevant lengths
...
pomoLength(); // setting text content

workMinutes.textContent = round(workMin);
breakMinutes.textContent = round(breakMin); // this sets new phases of the timer if a phase ends.

function pomodoroTimer(mode) {
  // update 'status' for user when pomo time changes
  pomoStatus.textContent = phase;
  peekStatusDisplay.textContent = phase; // check sessions
  ...
  if (sessions < 5) {
    ...
    pomoMin = 30;
    pomoSec = 0;
    phase = 'finalbreak'; // enable animations

    var _fullTime = pomoMin * 60 + pomoSec;

    finalLoadSpanContainer.style.display = 'flex';
    timeline.style.display = 'none';
    finalLoadSpan.style.animation = 'load-spans ' + _fullTime + 's ' + 'linear forwards';
  } else {
    pomoAlarm.play(); // otherwise, the pomodoro has ended - reset everything
...
function endPomo() {
  ...
  timeToolsOverlay.setAttribute('static', 'true'); // reset the pomo time
  ...
  pomoSeconds.textContent = round(pomoSec);
  peekMinutes.textContent = round(pomoMin);
  peekSeconds.textContent = round(pomoSec); // stop the countup intervals

  clearInterval(pomoIntervals); // reset the text

  pomoStartStop.textContent = 'Start';
  pomoStartStop.classList.remove('danger');
  pomoStartStop.classList.add('primary'); // allow dropdowns

  dropdown.addEventListener('click', toggleTimeMenu); // display/show relevant elements

  workButtons.style.display = 'flex';
  breakButtons.style.display = 'flex';
  timer.style.display = 'none';
  pomoOn = false; // remove animation functions
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","js/time.js"], null)
//# sourceMappingURL=/time.b749e876.js.map